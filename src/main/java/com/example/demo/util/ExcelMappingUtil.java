...
public class ExcelMappingUtil {
	...
	static {
		TYPE_CONVERTERS.put(Long.class, ExcelMappingUtil::parseLong);
		TYPE_CONVERTERS.put(long.class, ExcelMappingUtil::parseLong);

		TYPE_CONVERTERS.put(Integer.class, ExcelMappingUtil::parseInt);
		TYPE_CONVERTERS.put(int.class, ExcelMappingUtil::parseInt);
		...
		for (DateTimeFormatter formatter : DATE_FORMATTERS) {
			try {
				if (value.contains(":")) {
					...
					return Date
							.from(LocalDate.parse(value, formatter).atStartOfDay(ZoneId.systemDefault()).toInstant());
				}
